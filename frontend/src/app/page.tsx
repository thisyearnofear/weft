...
export default function Home() {
  ...
  return (
    <div className={styles.container}>
      ...
      {/* ════════════════════════════════════════════════════════════════
          AUTONOMOUS AGENT FLOW — what the daemon does, step by step.
          Shows judges and users that verification is fully autonomous,
          not a human clicking buttons.
          ════════════════════════════════════════════════════════════════ */}
      <Reveal as="section" className={styles.section} delay={100}>
        <div className={styles.sectionHeader}>
          <div>
            <span className={styles.sectionKicker}>Autonomous verification</span>
            <h2 className={styles.sectionTitle}>No one approves anything</h2>
          </div>
        </div>
        <p className={`${styles.sectionText} ${styles.sectionLede}`}>
        ...
        <div className={styles.agentFlowGrid}>
          <div className={styles.agentFlowStep}>
            <span className={styles.agentFlowNum}>01</span>
            <h3 className={styles.agentFlowTitle}>Poll</h3>
            <p className={styles.agentFlowBody}>
              The daemon watches the contract for milestones past their deadline.
              When it finds one, it reads the metadata hash from 0G Storage to
              learn what to verify — contract address, unique-caller threshold,
              measurement window.
            </p>
          </div>
          <div className={styles.agentFlowStep}>
            <span className={styles.agentFlowNum}>02</span>
            <h3 className={styles.agentFlowTitle}>Collect evidence</h3>
            <p className={styles.agentFlowBody}>
              It checks if the contract is deployed (<code>eth_getCode</code>),
              counts unique callers in the measurement window, and optionally
              pulls GitHub commits. The evidence is deterministic — same inputs,
              same verdict, every time.
            </p>
          </div>
          <div className={styles.agentFlowStep}>
            <span className={styles.agentFlowNum}>03</span>
            <h3 className={styles.agentFlowTitle}>Encrypt the verdict</h3>
            <p className={styles.agentFlowBody}>
              For confidential milestones, the daemon calls the Zama relayer SDK
              to encrypt its ballot as an <code>euint32</code>. For v2, it also
              encrypts a confidence score (1–100) derived from evidence strength.
              The vote leaves the process as ciphertext.
            </p>
          </div>
          <div className={styles.agentFlowStep}>
            <span className={styles.agentFlowNum}>04</span>
            <h3 className={styles.agentFlowTitle}>Submit sealed ballot</h3>
            <p className={styles.agentFlowBody}>
              The encrypted handle + Zama proof are submitted onchain via{" "}
              <code>submitVerdict</code> (v1) or <code>submitWeightedVerdict</code>{" "}
              (v2). The transaction is visible on Etherscan — but no one can read
              the vote from the calldata.
            </p>
          </div>
          <div className={styles.agentFlowStep}>
            <span className={styles.agentFlowNum}>05</span>
            <h3 className={styles.agentFlowTitle}>Homomorphic tally</h3>
            <p className={styles.agentFlowBody}>
              The contract adds encrypted votes (<code>FHE.add</code>) and, for v2,
              multiplies ballot × confidence (<code>FHE.mul</code>). Quorum is
              checked on ciphertext — <code>FHE.ge</code>, <code>FHE.and</code>.
              Only the final boolean is made decryptable, and only after all
              ballots are cast.
            </p>
          </div>
          <div className={styles.agentFlowStep}>
            <span className={styles.agentFlowNum}>06</span>
            <h3 className={styles.agentFlowTitle}>Release &amp; earn</h3>
            <p className={styles.agentFlowBody}>
              When quorum is reached, the contract releases ETH to the builder.
              The agent earns 3% — swept into its Stripe operating balance to pay
              for its own infrastructure. ENS reputation is updated. The story is
              written.
            </p>
          </div>
        </div>
      ...
      {/* ════════════════════════════════════════════════════════════════
          WHY FHE — the problem and the fix, in plain language.
          ════════════════════════════════════════════════════════════════ */}
      <Reveal as="section" className={styles.section} delay={100}>
        <div className={styles.sectionHeader}>
          <div>
            <span className={styles.sectionKicker}>Why sealed ballots</span>
            <h2 className={styles.sectionTitle}>The problem FHE solves</h2>
          </div>
        </div>
        <div className={styles.problemSolution}>
          <div className={styles.problemCard}>
            <span className={styles.problemKicker}>Without FHE</span>
            <h3 className={styles.problemTitle}>Verifier herding</h3>
          ...
          <div className={styles.solutionCard}>
            <span className={styles.solutionKicker}>With Zama FHE</span>
            <h3 className={styles.solutionTitle}>Sealed-ballot consensus</h3>
            <p className={styles.solutionBody}>
              Each agent encrypts its vote in its own process. The contract
              tallies homomorphically — arithmetic, comparison, and multiplication
              on data it can never read. No agent can see another&apos;s vote
              before quorum. Independence isn&apos;t a policy. It&apos;s math.
            </p>
          </div>
        </div>
      </Reveal>
