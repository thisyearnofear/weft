...
# Use ripgrep if available, otherwise fall back to POSIX extended grep.
# Patterns below are ERE-compatible ([[:space:]] instead of \s) so both
# engines accept them — grep -P is GNU/Linux-only and fails on macOS.
...
SECRET_PATTERNS=(
  ...
  'fal_key[=:][[:space:]]*["'\'']?[a-f0-9-]+:[a-f0-9-]+'  # fal.ai
  'KIMI_API_KEY[=:][[:space:]]*["'\'']?sk-[A-Za-z0-9]+'
  'VERIFIER_PRIVATE_KEY[=:][[:space:]]*["'\'']?0x[0-9a-fA-F]{64,}'
)
