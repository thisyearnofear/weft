...
class CardData:
    ...
    chapter_heading: str = ""
    chapter_body: str = ""
    image_url: str = ""           # fal.ai-generated woven swatch image
    image_prompt: str = ""        # the prompt used (shown for transparency)
...
def generate_milestone_card(data: CardData) -> str:
    ...
    image_html = (
        f'<img class="swatch-image" src="{html.escape(data.image_url)}" alt="AI-woven swatch — {heading}" />'
        if data.image_url else ""
    )
    image_caption = (
        f'<div class="swatch-caption">AI-woven swatch — fal.ai · prompt: <em>{html.escape(data.image_prompt[:140])}…</em></div>'
        if data.image_url and data.image_prompt else ""
    )
...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .swatch-caption em {{ color: #9090a8; }}
...
def generate_chronicle_html(
    ...
    cover_image_url: str = "",
    chapter_image_urls: Optional[List[str]] = None,
) -> str:
    """
    ...
    If `cover_image_url` is set (fal.ai-generated), it is shown as the hero
    above the title. If `chapter_image_urls` is provided, each chapter is
    paired with its corresponding generated swatch image.
    """
    chapter_image_urls = chapter_image_urls or []
    chapter_html = ""
    for i, ch in enumerate(chapters):
        heading = html.escape(ch.get("heading", f"Chapter {i+1}"))
        body = html.escape(ch.get("body", ""))
        img_url = chapter_image_urls[i] if i < len(chapter_image_urls) else ""
        img_html = (
            f'<img class="chapter-image" src="{html.escape(img_url)}" alt="Swatch for {heading}" />'
            if img_url else ""
        )
        chapter_html += f"""
    <div class="chapter">
    ...
    cover_html = (
        f'<img class="cover" src="{html.escape(cover_image_url)}" alt="Chronicle cover tapestry" />'
...
<div class="tapestry">
  ...
  <div class="subtitle">A Builder Journey — woven from onchain threads · imagery generated by fal.ai · narrative by Kimi</div>
  {chapter_html}
...
def write_chronicle(
    ...
    page = generate_chronicle_html(
        title, chapters, epilogue, attestations,
        cover_image_url=cover_image_url,
        chapter_image_urls=chapter_image_urls,
    )
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(page)
